...
import { localizeDigits, formatDateTime, translateApiError } from '@/lib/format-utils';
...
export function DocumentsSection({ companyId, isReadOnly = false }: DocumentsSectionProps) {
  const { t, i18n } = useTranslation('company-document');
  const locale = i18n.language === 'fa' ? 'fa-IR' : 'en-US';
  const langId = i18n.language === 'fa' ? 12 : 10;
  const [documents, setDocuments] = useState<DocumentItem[]>([]);
  const [documentTypes, setDocumentTypes] = useState<DocumentTypeTranslation[]>([]);
  ...
  // Unique document type IDs (sorted) for the dropdown
  const documentTypeIds = useMemo(() => {
    const ids = new Set<number>();
    documentTypes.forEach((d) => ids.add(d.companyDocumentTypeId));
    return Array.from(ids).sort((a, b) => a - b);
  }, [documentTypes]);

  const getDocTypeName = (id: number) => {
    const tr = documentTypes.find(
      (d) => d.companyDocumentTypeId === id && d.languageId === langId,
    );
    return tr?.name || '-';
  };
  ...
  // Load document types once (fa/en translations for the dropdown)
  useEffect(() => {
    let active = true;
    (async () => {
      try {
        ...
        if (active && Array.isArray(data)) setDocumentTypes(data);
      } catch {
  ...
  // Load documents for the selected company
  const loadDocuments = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/company/document?companyId=${companyId}`);
  ...
  const handleOpenEdit = (index: number) => {
    ...
    setFormDocTypeId(doc.companyDocumentTypeId);
    setFormFile(null);
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingIndex !== null) {
        ...
        formData.append('companyDocumentTypeId', String(formDocTypeId));
        if (formFile) formData.append('file', formFile);

        const response = await fetch('/api/company/document', {
          method: 'PUT',
          body: formData,
        });
        if (!response.ok) {
  ...
  const handleDelete = async (index: number) => {
    const doc = documents[index];
    try {
      await fetch('/api/company/document', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: doc.id, companyId }),
      });
      await loadDocuments();
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <span className="w-8 h-8 bg-amber-500 rounded-lg flex items-center justify-center text-white text-sm font-bold">8</span>
                  ...
                  <TableRow key={doc.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        {getDocTypeName(doc.companyDocumentTypeId)}
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <Button asChild variant="ghost" size="icon" className="h-8 w-8" title={t('common:view', { defaultValue: 'View' })}>
                          <a href={`/api/company/document/${doc.id}/file?companyId=${companyId}`} target="_blank" rel="noopener noreferrer">
                            <Eye className="h-4 w-4" />
                          </a>
                        </Button>
                        <Button asChild variant="ghost" size="icon" className="h-8 w-8" title={t('common:download', { defaultValue: 'Download' })}>
                          <a href={`/api/company/document/${doc.id}/file?companyId=${companyId}`} download={doc.fileName}>
